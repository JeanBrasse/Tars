...
A restart that waits is shown on the agent's panel, with what it waits on (`agent:restart-pending`,
and `agent:pendingRestarts` for a window opened since). A restart waiting on a field is waiting on you: send what is typed there, or clear it. Only the
