...
`spawnAgentSession` refuses to start a claude-binary alt provider that produced no `ANTHROPIC_BASE_URL` — that means no API key is configured, and the session would silently bill the user's Anthropic account:
