...
- **An agent's own token is not secret from the other agents.** It lives in its process environment, and `ps -Eww -p <pid>` prints the environment of any process of the same user. An agent set on it can read a colleague's token and present it. Per-agent tokens end impersonation by naming, not impersonation by a process that reads the process table; only isolating agents from one another at the OS level would.
