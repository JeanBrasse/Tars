...
- Auth: `~/.dorothy/hermes-webhook-secret` if present, with the master API token still accepted so an existing setup keeps running. This route is the one thing published over the tailnet, so it carries its own secret, and since 1.7.6 the door knows that secret too: it is authorised for this pathname and no other. Before that the webhook secret was refused with a flat 401 at the door, before the route that expects it could run.
...
| Auth | `Authorization: Bearer <~/.dorothy/api-token>`, 32 random bytes, file mode `0600`, or an agent's own token, minted in memory for each terminal spawn and each delegated run, or Tars's own pass, minted in memory and written nowhere, which the super chat presents on the loopback. The agent's token decides who is calling; with it, an `X-Tars-Caller-Id` naming another agent is a 403. The shared token names no agent, no header is read with it, and it drives no agent |
