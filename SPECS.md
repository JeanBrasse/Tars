...
| `amp`, global npm package | `npm view <package> version`, then the new version downloaded into a scratch prefix, then `npm install --global --prefix <prefix> --prefer-offline <package>@<version>` | while any process has the binary open (`lsof -t`), because npm removes the old package before the new one is in place |
...
- **A claude session that outlives two newer releases can lose its binary file.** The native installer's cleanup keeps the two newest versions and any version whose lock is held, and only the first session on a version holds that lock. Measured: once that session had exited, the cleanup deleted the file under a second session on the same version, and that session's next turn still answered. What would fail is claude starting itself again from inside it (the `claude` its Bash tool is given points at the running binary). A restart ends it. The cleanup is claude's own: every session's housekeeping runs it, not only an update.
