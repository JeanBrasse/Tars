...
export const CHANGELOG: Release[] = [
  {
    ...
    updates: [
      ...
      'When a changed setting is waiting to restart an agent, its panel says so and what the restart waits for: the end of a turn, an answer to a permission question, or what you are typing in its field',
      ...
      'A task delegated to an agent is no longer lost to its own limits: work it leaves running is reported when it ends, the orchestrator keeps listening while it waits, and the brief is never sent twice',
      ...
      'Each agent is drawn with a small mark made from its name, the same wherever it appears, and an orchestrator\'s mark is orange. Beside it, an agent\'s state is a word in its colour. The character picker is gone',
      'An orchestrator\'s window lists the other projects\' orchestrators, drawn in orange',
      ...
      'Looking at an agent that is not running no longer opens a shell in its terminal: its panel says (Session idle), and the Chat no longer shows that shell\'s banner as the agent\'s last words',
      ...
      'Settings, the new agent and new team dialogs, Review, Extensions and the project lists answer from what Tars already found. The login shell that finds your CLIs runs once per launch, and again when you press Detect in Settings > CLI Paths, instead of on every section and dialog; a branch\'s diff is reused until a file or a branch moves; the skills marketplace shows its last listing at once and refreshes it once it is an hour old',
      ...
      'After you change the Hermes connection, sign in or sign out, the Chat goes back to its live Hermes session instead of the slower path it kept until Tars restarted. Hermes errors read as Hermes wrote them, the Chat\'s effort picker included, where some showed [object Object], and the Hermes page gives the webhook address on this Tars\'s own port',
      'Telegram starts an agent that is not running again: /start_agent, and a message to the orchestrator while no CLI runs, launched nothing and left the agent marked running',
