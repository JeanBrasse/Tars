...
export const CHANGELOG: Release[] = [
  {
    ...
    updates: [
      ...
      'In the Chat, what you were typing in one room no longer follows you to the next, where Enter sent it to that room\'s agents. An agent on a CLI that never reports its turns, such as grok or codex, shows in the room\'s team why it stopped on an error, where it read "no turn signal"',
      ...
      'Run from source on Linux: the open button beside the config folder in Settings > System opens a terminal there, the first installed of x-terminal-emulator, gnome-terminal, konsole and xterm, where it did nothing; a terminal Tars opens runs bash when SHELL is unset, where it failed without zsh; and the status line reuses its git cache, as on macOS',
    ],
  },
  {
    id: 47,
