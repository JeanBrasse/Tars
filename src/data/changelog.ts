...
export const CHANGELOG: Release[] = [
  {
    ...
    updates: [
      ...
      'The orders that went missing were being done all along. Every agent asks Tars whether it is running before it reports anything, and that question had a time limit on reaching the app but none on hearing back. Against a port that answers the knock and then says nothing, which is what a second Tars, a leftover process or a sandbox on the same port looks like, the question simply never returned. It is the first thing an agent does when it starts, so it never got as far as telling Tars which session it was, and everything it said from then on was filed under nobody and thrown away. The task ran, the work happened, and the result went nowhere. The question now gives up after two seconds and the agent carries on either way, so a busy or wedged port costs a couple of seconds instead of the whole session',
