...
export const CHANGELOG: Release[] = [
  {
    ...
    date: '2026-09-17',
    updates: [
      'Tars no longer rewrites Claude\'s own files in place. Your Claude settings, ~/.claude.json and ~/.claude/mcp.json are read by every Claude session that runs, and Tars wrote into them directly: the status line did it at every launch, even with nothing to change. A session reading at the wrong moment could meet a half-written file, and a file Tars could not read was replaced by only the part Tars meant to add. These files are now written beside the original and swapped in whole, keep their permissions, are left alone when nothing changes, and are never replaced when Tars cannot read them. The Kanban board file is written the same way, by Tars and by the agents\' Kanban server',
