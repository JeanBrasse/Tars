...
export const CHANGELOG: Release[] = [
  {
    ...
    updates: [
      ...
      'Scrolling a terminal panel works, and keeps working. Claude Code asks for the alternate screen in its very first frame and never gives it back, and a terminal in the alternate screen has no history by definition, so the wheel was quietly turned into arrow key presses aimed at the CLI instead of moving the panel. It looked random because a panel only landed there when the saved output it replays as it opens still carried that first frame: an agent that had said a lot had already pushed it out, a quiet or freshly started one had not. That is also why going full screen appeared to repair it, and why now and then it took two goes. Tars refuses the alternate screen now, the way it already refuses the mouse, so every panel keeps its ten thousand lines of history and the wheel reaches all of them',
      'Hold Option and click to put the cursor where you clicked. It has been there the whole time and nobody knew: it is the reason Tars will not hand the mouse to the CLI, because a terminal that gives the mouse away loses the wheel, the selection and this at once. Correcting the middle of a long message no longer means holding an arrow key down, and the same move measured four hundred and sixty two presses when it was counted. It works while the panel is at the bottom of its history, which is where you type',
