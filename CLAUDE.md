...
- **Design rule**: the frames in `design/tars-redesign.pen`, plus `design/chat-design.pen` for the Chat room and `design/chat-redesign-a.pen` for its redesign, and the tokens in `DESIGN.md` are the specification. Never invent a layout, a colour, or a control height. Controls are **26px** (small) or **32px** (standard), nothing else. The shell is identical on every page: sidebar 240 wide (72 collapsed), header 84 tall with padding `22/26/14/26`, content `0/26/22/26`
...
- **Auth rule**: exactly two routes are exempt from the bearer token: `/api/local-file` and `/api/health`. Anything you add under `/api/` is authenticated. `/api/hooks/*` was exempt until 2026-09-23 and now takes the agent's own token (`CLAUDE_MGR_API_TOKEN`, which the hooks inherit from the CLI they run in), for the `agent_id` it names and no other: the shared token is refused there. It was four until `/api/kanban/complete` was removed: unauthenticated, uncalled, and it deleted agents
