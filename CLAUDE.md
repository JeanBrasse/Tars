...
- **Design rule**: the frames in `design/tars-redesign.pen`, plus `design/chat-design.pen` for the Chat room, `design/chat-redesign-a.pen` for its redesign and `design/landing.pen` for the landing site, and the tokens in `DESIGN.md` are the specification. Never invent a layout, a colour, or a control height. Controls are **26px** (small) or **32px** (standard), nothing else. The shell is identical on every page: sidebar 240 wide (72 collapsed), header 84 tall with padding `22/26/14/26`, content `0/26/22/26`
