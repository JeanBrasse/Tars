...
| `electron/handlers/ipc-handlers.ts` | 2982 lines, nearly every `ipcMain.handle`. Start here when a renderer call has no backend |
...
| `design/tars-redesign.pen` | Pencil source of truth, 87 root frames. Written by the `pen` CLI as plain JSON: draw through the CLI (Workflow Rule 1), never the `pencil` MCP tools |
